...
function computeMacroTargets(profile) {
  ...
  const protein_min_g = 0.8 * weight_kg;
  const protein_max_g = 2.2 * weight_kg;
  const fat_min_g = 0.5 * weight_kg;
  const fat_max_g = (0.35 * goal_calories) / 9;

  const carbs_max_g = Math.max(0, (goal_calories - (protein_min_g * 4 + fat_min_g * 9)) / 4);
  const carbs_min_g = Math.max(0, (goal_calories - (protein_max_g * 4 + fat_max_g * 9)) / 4);
  ...
  return {
    ...
    floor_applied,
    protein: {
      min_g: round1(protein_min_g),
      max_g: round1(protein_max_g),
      min_kcal: Math.round(protein_min_g * 4),
      max_kcal: Math.round(protein_max_g * 4),
    },
    fat: {
      min_g: round1(fat_min_g),
      max_g: round1(fat_max_g),
      min_kcal: Math.round(fat_min_g * 9),
      max_kcal: Math.round(fat_max_g * 9),
    },
    carbs: {
      min_g: round1(carbs_min_g),
      max_g: round1(carbs_max_g),
      min_kcal: Math.round(carbs_min_g * 4),
      max_kcal: Math.round(carbs_max_g * 4),
    },
    fiber: {
      min_g: fiber_min_g,
    },
